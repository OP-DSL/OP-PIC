...
//--------------------------------------------------------------
__global__ void sequence_OPP_INT_values( // Used for Segmented Reductions
...
//--------------------------------------------------------------
__global__ void reset_OPP_INT_values( // Used for Segmented Reductions
    OPP_INT *__restrict values,
    const int start,
    const int end) 
{
...
//--------------------------------------------------------------
__global__ void reset_OPP_REAL_values( // Used for Segmented Reductions
        ...
        do
        {
            ...
            if (on_old_cell)
            {
                ...
                for (int d = 0; d < 12; ++d, ++offset) {
                    ...
                    sr_dat5_keys[n + opp_k2_sr_set_stride_d * offset] = p2c; // TODO : Generate for double indirections too!
                }
            }
            else
            {
...
opp_profiler->end("SR_Resize");

// opp_profiler->start("SR_Init1");
//             thrust::fill(sr_dat5_keys_dv.begin(), sr_dat5_keys_dv.end(), 0);
// opp_profiler->end("SR_Init1");
// opp_profiler->start("SR_Init3");
//             thrust::fill(sr_dat5_values_dv.begin(), sr_dat5_values_dv.end(), 0);
// opp_profiler->end("SR_Init3");

opp_profiler->start("SR_Init13");
            opp_k2::reset_OPP_INT_values<<<num_blocks, block_size>>>(
                (OPP_INT *)thrust::raw_pointer_cast(sr_dat5_keys_dv.data()),
                0, sr_dat5_keys_dv.size());
            opp_k2::sequence_OPP_INT_values<<<num_blocks, block_size>>>(
                (OPP_INT *)thrust::raw_pointer_cast(sr_dat5_keys_dv2.data()),
                0, sr_dat5_keys_dv2.size());
            ...
            opp_k2::reset_OPP_REAL_values<<<num_blocks2, block_size>>>(
                ...
                0, sr_dat5_values_dv.size());
            opp_k2::reset_OPP_REAL_values<<<num_blocks2, block_size>>>(
                (OPP_REAL *)thrust::raw_pointer_cast(sr_dat5_values_dv2.data()),
                0, sr_dat5_values_dv2.size());
            cutilSafeCall(hipDeviceSynchronize());
opp_profiler->end("SR_Init13");
...
//                     sr_dat5_keys_dv.begin() + d * opp_k2_sr_set_stride, 
//                     sr_dat5_keys_dv.begin() + (d + 1) * opp_k2_sr_set_stride, 
//                     sr_dat5_values_dv.begin() + d * opp_k2_sr_set_stride);
//             }
...
//                     (OPP_INT *) thrust::raw_pointer_cast(sr_dat5_keys_dv.data() + d * opp_k2_sr_set_stride),
