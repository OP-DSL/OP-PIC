...
for run in 1; do
    for ppc in 105 210; do
        for dt in 0.5 2.5 4.5 7.0; do
            ...
            srun gpu_launch.sh ${binary} ${currentfilename} > $folder/log_G16_M${mesh}_D${ppc}_ARR${dt}_H0_R${run}.log;

            sed -i "s|BOOL opp_global_move = false|BOOL opp_global_move = true|" ${currentfilename}

            srun gpu_launch.sh ${binary} ${currentfilename} > $folder/log_G16_M${mesh}_D${ppc}_ARR${dt}_H1_R${run}.log;
        done
    done
done
