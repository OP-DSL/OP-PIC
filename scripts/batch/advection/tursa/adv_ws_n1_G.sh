...
#SBATCH --gres=gpu:1            # Change the gpus variable as well
##SBATCH --exclusive
...
runFolder=$PWD"/MPI_N"${SLURM_JOB_NUM_NODES}"_G"${gpus}"_SR"${use_seg_red}"_"$(date +"D_%Y_%m_%d_T_%I_%M_%S")
...
for run in 1; do
    for ppc in 105 210; do
        for dt in 0.5 2.5 4.5 7.0; do
            ...
            (( actual_ny=$mesh*$SLURM_JOB_NUM_NODES*$gpus ))
            ...
            srun gpu_launch.sh ${binary} ${currentfilename} > $folder/log_G${gpus}_M${mesh}_D${ppc}_ARR${dt}_H0_R${run}.log;

            sed -i "s|BOOL opp_global_move = false|BOOL opp_global_move = true|" ${currentfilename}

            srun gpu_launch.sh ${binary} ${currentfilename} > $folder/log_G${gpus}_M${mesh}_D${ppc}_ARR${dt}_H1_R${run}.log;
        done
    done
done
