...
inline void gen_dh_structured_mesh(opp_set set, const opp_dat c_gbl_id, opp_map c2c_map,
    ...
    if (OPP_rank == 0)            
        opp_printf("APP", "gen_dh_structured_mesh START cells [%s] global grid dims %d %d %d",
        ...
        for (int dy = cellMapper->localGridStart.y; dy < cellMapper->localGridEnd.y; dy++) {            
            ...
            for (int dx = cellMapper->localGridStart.x; dx < cellMapper->localGridEnd.x; dx++) {                
                ...
                if (cid == MAX_CELL_INDEX) {
                    ...
                    // if (((int*)c_gbl_id->data)[cid] == 14874)
                    //     opp_printf("YYYY", "gcid %d rank %dx", ((int*)c_gbl_id->data)[cid], OPP_rank);
                }
            }
        }
    }
...
//     if (cellMapper->structMeshToCellMapping[i] == 14874 && cellMapper->structMeshToRankMapping[i] == 0)
//         opp_printf("ZZZ", "%d cell %d rank_map %d rank %d", i, cellMapper->structMeshToCellMapping[i], cellMapper->structMeshToRankMapping[i], OPP_rank);
// }
// }
// cellMapper->waitBarrier();
    ...
    for (auto& p : removed_coords) {
        ...
        if ((most_suitable_gbl_cid != MAX_CELL_INDEX) && (most_suitable_gbl_cid < avail_gbl_cid) && 
            ...
            // if (most_suitable_gbl_cid == 14874)
            //     opp_printf("ZZZ", "%d cell %d rank_map %d rank %d", 
            //         index, cellMapper->structMeshToCellMapping[index], cellMapper->structMeshToRankMapping[index], OPP_rank);
