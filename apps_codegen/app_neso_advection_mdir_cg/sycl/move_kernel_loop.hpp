...
void opp_dev_move_kernel_sycl(opp_set set, const int nargs, opp_arg *args, opp_map p2c_map, opp_map c2c_map) {
    ...
    opp_queue->submit([&](sycl::handler &cgh) {
            ...
            const double* p_pos, int* p_mdir, const double* c_pos_ll)
        {
            ...
            {
                // Find whether the final position is towards positive or negative direction
                const bool is_x_positive = (((p_pos_x_diff > CONST_cell_width_sycl[0]) &&
                                            (CONST_extents_sycl[Dim::x] > 2 * p_pos_x_diff - CONST_cell_width_sycl[0])) ||
                                            ((p_pos_x_diff < 0) && (CONST_extents_sycl[Dim::x] < CONST_cell_width_sycl[0] - 2 * p_pos_x_diff)));

                const bool is_y_positive = (((p_pos_y_diff > CONST_cell_width_sycl[0]) &&
                                            (CONST_extents_sycl[Dim::y] > 2 * p_pos_y_diff - CONST_cell_width_sycl[0])) ||
                                            ((p_pos_y_diff < 0) && (CONST_extents_sycl[Dim::y] < CONST_cell_width_sycl[0] - 2 * p_pos_y_diff)));

                p_mdir[(Dim::x) * opp_k2_dat1_stride_sycl[0]] = is_x_positive ? 1 : -1;
                p_mdir[(Dim::y) * opp_k2_dat1_stride_sycl[0]] = is_y_positive ? 1 : -1;
            }
            // Step X : end
            ...
            if ((p_pos_x_diff >= 0.0) && (p_pos_x_diff <= CONST_cell_width_sycl[0])) {
                ...
                opp_p2c[0] = opp_c2c[(CellMap::xu_y) * opp_k2_c2c_map_stride_sycl[0]]; // Step (5) : Calculate the next most probable cell index to search
                { opp_move_status_flag = OPP_NEED_MOVE; }; return;
            }
            else if (p_mdir[(Dim::x) * opp_k2_dat1_stride_sycl[0]] < 0) {
                opp_p2c[0] = opp_c2c[(CellMap::xd_y) * opp_k2_c2c_map_stride_sycl[0]]; // Step (5) : Calculate the next most probable cell index to search
                { opp_move_status_flag = OPP_NEED_MOVE; }; return;
            }
            ...
            if ((p_pos_y_diff >= 0.0) && (p_pos_y_diff <= CONST_cell_width_sycl[0])) {
                ...
                opp_p2c[0] = opp_c2c[(CellMap::x_yu) * opp_k2_c2c_map_stride_sycl[0]]; // Step (5) : Calculate the next most probable cell index to search
                { opp_move_status_flag = OPP_NEED_MOVE; }; return;
            }
            else if (p_mdir[(Dim::y) * opp_k2_dat1_stride_sycl[0]] < 0) {
                opp_p2c[0] = opp_c2c[(CellMap::x_yd) * opp_k2_c2c_map_stride_sycl[0]]; // Step (5) : Calculate the next most probable cell index to search
                { opp_move_status_flag = OPP_NEED_MOVE; }; return;
            }
