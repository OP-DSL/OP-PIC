...
#SBATCH --job-name=fem_energy
#SBATCH --time=02:00:00
#SBATCH --partition=gpu-a100-80
#SBATCH --account=dp360
#SBATCH --qos=standard

#SBATCH --nodes=8           # Controlled by node_count variable below
...
node_count=6
runFolder=$PWD"/CUDA_ENERGY_"$node_count"_"$(date +"D_%Y_%m_%d_T_%I_%M_%S")
...
for gpus in 4; do
    for config in 1536000 3072000; do
        ...
        for gpu_red_arrays in 256 512; do
            for run in 1 2; do
                echo "Running CUDA BLOCK " $config $run $gpus $gpu_red_arrays
                ...
                sed -i "s|STRING hdf_filename = <path_to_hdf5_mesh_files>/box_48000.hdf5|STRING hdf_filename = /home/dp360/dp360/dc-lant1/phd/box_mesh_gen/hdf5/box_${config}.hdf5|" ${currentfilename}
                ...
                (( partitions=$gpus*$node_count ))
                sed -i "s/INT opp_partitions = 1/INT opp_partitions = ${partitions}/" ${currentfilename}

                srun --hint=nomultithread --distribution=block:block --ntasks=$tasks $binary $currentfilename > $folder/log_G${gpus}_M${config}_D${plasma_den}_ARR${gpu_red_arrays}_R${run}.log;
            done
        done
    done
done
