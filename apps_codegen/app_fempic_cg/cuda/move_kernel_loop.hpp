...
__device__ inline void move_kernel(
    ...
    // Step (2) : A method to identify if the particle has reached its final mesh cell
    if ((point_lc[(0) * opp_k4_dat1_stride_d] > (-1) * 1e-10) && ((point_lc[(0) * opp_k4_dat1_stride_d] - 1.0) < 1e-10) &&
        (point_lc[(1) * opp_k4_dat1_stride_d] > (-1) * 1e-10) && ((point_lc[(1) * opp_k4_dat1_stride_d] - 1.0) < 1e-10) &&
        (point_lc[(2) * opp_k4_dat1_stride_d] > (-1) * 1e-10) && ((point_lc[(2) * opp_k4_dat1_stride_d] - 1.0) < 1e-10) &&
        (point_lc[(3) * opp_k4_dat1_stride_d] > (-1) * 1e-10) && ((point_lc[(3) * opp_k4_dat1_stride_d] - 1.0) < 1e-10)) {
    ...
    if (opp_c2c[(min_i) * opp_k4_c2c_map_stride_d] >= 0) { // is there a neighbor in this direction?
        ...
        (*opp_p2c) = opp_c2c[(min_i) * opp_k4_c2c_map_stride_d]; // Assign new p2c mapping
        { opp_move_status_flag = OPP_NEED_MOVE; };
    }
    else {
        // Step (4) : Actions to be carried out if the particle has moved out of the mesh domain
