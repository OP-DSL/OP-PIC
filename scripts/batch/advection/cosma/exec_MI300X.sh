...
file=$HOME/phd/OP-PIC/scripts/batch/advection/cosma/$configFile
...
for gpus in 1 2 4 8 ; do
    for run in 1 2; do
        for ppc in 425 850 1700; do
            ...
            srun --ntasks=$gpus --distribution=block:block --hint=nomultithread --unbuffered ${binary} ${currentfilename} | tee $folder/log_G${gpus}_M${mesh}_D${ppc}_ARR1_R${run}.log;
        done
    done
done
